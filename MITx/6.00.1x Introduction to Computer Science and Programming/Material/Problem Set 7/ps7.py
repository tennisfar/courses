...
class WordTrigger(Trigger):
    ...
    def isWordIn(self,text):
        ...
        for i in range(len(text)):
            if self.text[i] in string.punctuation:
...
def makeTrigger(triggerMap, triggerType, params, name):
    ...
    if triggerType == 'title':
        triggerMap.update({name:TitleTrigger(params)})
        return TitleTrigger(params)
    if triggerType == 'subject':
        triggerMap.update({name:SubjectTrigger(params)})
...
def main_thread(master):
    ...
    try:
        ...
        triggerlist = readTriggerConfig("/Users/MLP/Google Drev/edX/6.00.1x Python/Problem Set 7/triggers.txt")
